...
namespace HR_Department_Project.DAL
{
    class DALStaff_information 
    {
        ...
        public void DALcreate(int id, int insurances_id,int marriage_status,  string position, string contracttype, DateTime dayofwork, DateTime signing_date, DateTime expiry_date, int department_id,int persional_infor, int employee_payroll)
        {
            DataTable dt = commonClass.LoadData($"SELECT * FROM personal_informations WHERE personal_informations.id = {persional_infor}");
            string place_of_issue = dt.Rows[0]["address"].ToString();
            DateTime date_of_issue = DateTime.Now;
            string sqlCreate = $"insert into staff_informations values ({id},'{marriage_status}',N'{position}',N'{contracttype}','{dayofwork}','{signing_date}', '{expiry_date}', {department_id}, {persional_infor}, {employee_payroll})";
            string sql = $@"insert into insurances values ({insurances_id}, '{date_of_issue}', N'{place_of_issue}', {persional_infor})";
