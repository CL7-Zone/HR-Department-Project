...
namespace HR_Department_Project.DAL
{
    class DALProbationaryRecord
    {
        ...
        public void DALcreate2(int id, int insurances_id, string probationary_position, DateTime probationary_date, DateTime expiry_date, string note, int department_id, int probationary_payroll_id, int persional_information_id)
        {
            DateTime date_of_issue = DateTime.Now;
            DataTable dt = commonClass.LoadData($"SELECT * FROM personal_informations WHERE personal_informations.id = {persional_information_id}");

            string sqlCreate = $"insert into probationary_records values ({id},N'" + probationary_position + "','" + probationary_date + "' ,'" + expiry_date + "', '" + note + "', " + department_id + ", " + probationary_payroll_id + ", " + persional_information_id + ")";
            string place_of_issue = dt.Rows[0]["address"].ToString();
            string sql = $@"insert into insurances values ({insurances_id}, '{date_of_issue}', N'{place_of_issue}', {persional_information_id})";
